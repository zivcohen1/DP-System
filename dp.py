...
class DPDSL_Rewriter_Visitor(DPDSLVisitor):
    ...
    def visitJoinTable(self, ctx):
        """Handle JOIN table sources"""
        self.has_join = True
    ...
    def visitAggregation(self, ctx):
        ...
        # Check if the inner expression is a ColumnReference with a label
        if type(inner_expr).__name__ == 'ColumnReferenceContext':
            label_ctx = inner_expr.label()
            if label_ctx:
                label = label_ctx.getText().upper()
                
                if label == 'PRIVATE':
                    col_ref = inner_expr.column_ref()
                    col_name = self._extract_column_name_from_ref(col_ref)
                    ...
                    # Rewrite: clip column value (handle table.column notation)
                    col_ref_ids = col_ref.identifier()
                    if len(col_ref_ids) == 2:
                        # table.column format
                        new_col_sql = f"MIN({col_ref_ids[0].getText()}.{col_ref_ids[1].getText()}, {base_sensitivity})"
                    else:
                        # just column format
                    ...
                    # Remove PUBLIC label but keep column reference
                    col_ref = inner_expr.column_ref()
                    col_ref_text = self._get_column_ref_text(col_ref)
                    self.rewriter.replaceRange(
                        inner_expr.start.tokenIndex,
                        inner_expr.stop.tokenIndex,
                        col_ref_text
    ...
    def visitColumnReference(self, ctx):
        ...
        # This is a direct column selection (not in aggregation)
        if ctx.label():
            label = ctx.label().getText().upper()
            col_ref = ctx.column_ref()
            col_name = self._extract_column_name_from_ref(col_ref)
            
            if label == 'PUBLIC':
                # Remove PUBLIC label but keep column
                col_ref_text = self._get_column_ref_text(col_ref)
                self.rewriter.replaceRange(ctx.start.tokenIndex, ctx.stop.tokenIndex, col_ref_text)
            elif label == 'PRIVATE':
                self.errors.append(
    ...
    def visitGroup_by_clause(self, ctx):
        ...
        for item_ctx in ctx.group_item():
            if item_ctx.label():
                label = item_ctx.label().getText().upper()
                col_ref = item_ctx.column_ref()
                col_name = self._extract_column_name_from_ref(col_ref)
                ...
                if label == 'PRIVATE':
                    ...
                    col_ref_text = self._get_column_ref_text(col_ref)
                    self.rewriter.replaceRange(
                        item_ctx.start.tokenIndex,
                        item_ctx.stop.tokenIndex,
                        col_ref_text
                    )
        return self.visitChildren(ctx)
    
    def _extract_column_name_from_ref(self, col_ref_ctx):
        """Extract column name from column_ref context (handles table.column)"""
        identifiers = col_ref_ctx.identifier()
        if len(identifiers) == 2:
            # table.column format - return just the column name
            return identifiers[1].getText()
        else:
            # just column format
            return identifiers[0].getText()
    
    def _get_column_ref_text(self, col_ref_ctx):
        """Get full column reference text (e.g., 'table.column' or 'column')"""
        identifiers = col_ref_ctx.identifier()
        if len(identifiers) == 2:
            return f"{identifiers[0].getText()}.{identifiers[1].getText()}"
        else:
            return identifiers[0].getText()
...
class DPDSLRewriter:
    ...
    def execute_query(self, query: str, user_id: str, verbose: bool = False) -> Tuple[Optional[List], Optional[str]]:
        ...
        try:
            ...
            # Database errors - provide detailed message for debugging
            ...
            # Unexpected errors - provide detailed message for debugging
